...
import os
import datetime
import json  # Para convertir dict <-> JSON
...
def audit_run(tid):
    ...
    if request.method == 'POST':
        ...
        rec = AuditRecord(
            ...
            user_id=current_user.id,
            answers=json.dumps(answers)  # <- Convertimos dict a JSON
        )
        db.session.add(rec)
        db.session.commit()
...
def audit_records():
    if current_user.role == 'empresa':
        # La empresa solo ve auditorías asociadas a sus empresas
    ...
    for rec in records:
        # Relacionar objetos para la plantilla
        ...
        # Convertir JSON string a dict solo para mostrar
        if rec.answers:
            try:
                rec.display_answers = json.loads(rec.answers)
            except:
                rec.display_answers = {}
        else:
            rec.display_answers = {}
